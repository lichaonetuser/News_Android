(function() {
	...
	window.bridge = function() {
		var callbacks = [],
			callbackID = 0,
			registerHandlers = [];
		document.addEventListener("JSBridgeDidReceiveNativeCallback", function(e) {
			if (e.detail) {
				var detail = e.detail;
		...
		document.addEventListener("JSBridgeDidReceiveNativeBroadcast", function(e) {
			if (e.detail) {
				var detail = e.detail;
		...
		return {
			"post": function(action, parameters, callback, print) {
				...
				if (typeof WebViewJSBridge != 'undefined') {
					var jsObj={};
					jsObj.action=action;
					jsObj.parameters=parameters;
					jsObj.callback=id;
					jsObj.print=print || 0;
					var str = JSON.stringify(jsObj);
					WebViewJSBridge.postMessage(str)
				}
			},
