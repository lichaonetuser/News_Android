...
import kotlinx.android.synthetic.main.item_admob_native_ad_right_img.view.*

open class AdMobNativeAdRightImgItem<Bean : INativeAdConfig>(mBean: Bean) : BaseItem<Bean, AdMobNativeAdRightImgItem.ViewHolder>(mBean) {

    private var mNativeAd: INativeAd? = null

    override fun getLayoutRes() = R.layout.item_admob_native_ad_right_img
    ...
    private fun bindViewHolder(holder: ViewHolder, unifiedNativeAd: UnifiedNativeAd) {
        ...
        ImageManager.with(holder.ad_primary_img)
//                // 裁剪比例不超过20%
...
//                        val ratio2 = holder.ad_primary_img.width.toFloat() / holder.ad_primary_img.height.toFloat()
//                        val maxRatio = ratio2 + (ratio2 * 0.18f)
//                        if (ratio >= ratio2 || ratio < maxRatio) {
//                            holder.ad_primary_img.hierarchy.actualImageScaleType = ScalingUtils.ScaleType.CENTER_CROP
//                        } else {
//                            holder.ad_primary_img.hierarchy.actualImageScaleType = ScalingUtils.ScaleType.FIT_CENTER
//                        }
//                    }
//                })
                .load(unifiedNativeAd.images?.getOrNull(0)?.uri?.toString())
