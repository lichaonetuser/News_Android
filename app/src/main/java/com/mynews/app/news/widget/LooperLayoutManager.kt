...
class LooperLayoutManager(conntext: Context) : LinearLayoutManager(conntext, VERTICAL, false) {
    private val TAG = "LooperLayoutManager"
    ...
    private fun fill(dy: Int, recycler: RecyclerView.Recycler?, state: RecyclerView.State?): Int {
        ...
        if (dy > 0) {
            ...
            if (lastView.bottom < height) {
                ...
                layoutDecorated(scrap, 0, lastView.bottom,
            ...
            if (firstView.top >= 0) {
                ...
                val width = getDecoratedMeasuredWidth(scrap)
                val height = getDecoratedMeasuredHeight(scrap)
                layoutDecorated(scrap, 0, firstView.top - height,
                        width, firstView.top)
            }
        }
        return dy
