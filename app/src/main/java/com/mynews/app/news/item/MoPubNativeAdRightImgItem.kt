...
import kotlinx.android.synthetic.main.item_mopub_native_ad_right_img.view.*

open class MoPubNativeAdRightImgItem<Bean : INativeAdConfig>(mBean: Bean) : BaseItem<Bean, MoPubNativeAdRightImgItem.ViewHolder>(mBean) {

    private var mNativeAd: INativeAd? = null
    private var mAdapterHelper: AdapterHelper? = null
    private var mNativeAdView: View? = null

    override fun getLayoutRes() = R.layout.item_mopub_native_ad_right_img

    override fun bindViewHolder(adapter: CommonRecyclerAdapter, holder: ViewHolder, position: Int, payloads: MutableList<Any?>?) {
        if (mNativeAd == null) {
            val googlePlayServicesAdRenderer = GooglePlayServicesAdRenderer(
                    MediaViewBinder.Builder(R.layout.item_mopub_native_ad_mediation_admob_right)
            ...
            val facebookAdRenderer = FacebookAdRenderer(
                    FacebookAdRenderer.FacebookViewBinder.Builder(R.layout.item_mopub_native_ad_mediation_facebook_right)
                            .titleId(R.id.ad_headline_txt)
            ...
            val moPubStaticNativeAdRenderer = MoPubStaticNativeAdRenderer(
                    ViewBinder.Builder(R.layout.item_mopub_native_ad_right_img_internal)
    ...
    class ViewHolder(view: View, adapter: CommonRecyclerAdapter, val mBean: INativeAdConfig) : BaseViewHolder(view, adapter) {
        ...
        private val mPlaceHolderVIew = LayoutInflater.from(view.context).inflate(R.layout.item_mopub_native_ad_right_img_internal, null)
