package com.mynews.common.extension.widget.bar.indicator.buildins.commonnavigator;
...
import com.mynews.common.core.widget.CoreHorizontalScrollView;
import com.mynews.common.extension.widget.bar.indicator.NavigatorHelper;
import com.mynews.common.extension.widget.bar.indicator.ScrollState;
...
import com.mynews.common.extension.widget.bar.indicator.buildins.commonnavigator.abs.IMeasurablePagerTitleView;
...
import com.mynews.common.extension.widget.bar.indicator.buildins.commonnavigator.model.PositionData;
...
public class CommonNavigator extends FrameLayout implements IPagerNavigator, NavigatorHelper.OnNavigatorScrollListener {
    private CoreHorizontalScrollView mScrollView;
    ...
    //滑动到给定的横坐标
    public void scrollTo(int loc){
        mScrollView.smoothScrollTo(loc, 0);
    }
    ...
    private void init() {
        ...
        View root;
        if (mAdjustMode) {
            root = LayoutInflater.from(getContext()).inflate(R.layout.core_widget_indicator_layout_navigator_no_scroll, this);
        } else {
            root = LayoutInflater.from(getContext()).inflate(R.layout.core_widget_indicator_layout_navigator, this);
            mScrollView = (CoreHorizontalScrollView) root.findViewById(R.id.scroll_view);   // mAdjustMode为true时，mScrollView为null
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mAdapter != null) {
            ...
            if (onLayoutFinishListener != null) {
                onLayoutFinishListener.onFinish();
            }
        }
    }
    ...
    public int getWholeWidth() {
        ...
        if (count == 0) {
            ...
            return mTitleContainer.getChildAt(count - 1).getRight();
        }
    }

    //获取给定子元素的左边缘的横坐标
    public int getChildLeftAt(int index) {
        return mTitleContainer.getChildAt(index).getLeft();
    }

    //获取给定子元素的右边缘的横坐标
    public int getChildRightAt(int index) {
        return mTitleContainer.getChildAt(index).getRight();
    }

    public void setOnScrollListener(CoreHorizontalScrollView.OnScrollListener onScrollListener) {
    ...
    public @Nullable View getTitleViewAt(int postion) {
        return mTitleContainer.getChildAt(postion);
    }
